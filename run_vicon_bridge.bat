...
wsl.exe -d Ubuntu-24.04 -- bash -ic "cd /home/aurora3var/vicon_receiver_ws && source /opt/ros/jazzy/setup.bash && export ROS_DOMAIN_ID=77 && echo '[1/3] Building workspace...' && colcon build --symlink-install --cmake-args -DCMAKE_BUILD_TYPE=Release -DCMAKE_EXPORT_COMPILE_COMMANDS=ON && echo '[2/3] Sourcing workspace...' && source install/setup.bash && echo '[3/3] Launching Vicon receiver...' && ros2 launch vicon_receiver client.launch.py hostname:=192.168.10.1 topic_namespace:=vicon buffer_size:=200 world_frame:=map vicon_frame:=vicon map_xyz:='[0.0, 0.0, 0.0]' map_rpy:='[0.0, 0.0, 0.0]' map_rpy_in_degrees:=false"
